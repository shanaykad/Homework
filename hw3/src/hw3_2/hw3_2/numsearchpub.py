...
def main(args=None):
    ...
    node = rclpy.create_node('numsearchpubnode')
    ...
    while rclpy.ok():
        ...
        msg1.data = [i] + randList
        msg2.data = [i] + randInt
        i += 1
        node.get_logger().info('Publishing list: "%s"' % msg1.data)
        node.get_logger().info('Publishing integer: "%s"' % msg2.data)
